    ...
    const minimalConfig = {
      ...
      module: {
        rules: [
          {
            test: /\.jsx?$/,
            exclude: /node_modules[\\/](?!(obj-to-css)[\\/])/,
            loader: 'babel-loader',
            options: {
              presets: [env],
            },
...
const setWebpackConfig = (goblin, outputPath, debugPath, target) => {
...
Goblin.registerQuest(goblinName, 'pack', function*(
  ...
  const cfg = setWebpackConfig(goblin, outputPath, debugPath, options.target);
...
Goblin.registerQuest(goblinName, 'server.start', function*(
  ...
  const cfg = setWebpackConfig(goblin, null, null, options.target);
  quest.goblin.setX('config', cfg);
  const config = cfg.toJS();
...
Goblin.registerQuest(goblinName, 'dist', function*(quest, outputPath) {
