...
const setWebpackConfig = (goblin, outputPath, target) => {
...
Goblin.registerQuest(goblinName, 'pack', function*(
  ...
  const cfg = setWebpackConfig(goblin, outputPath, options.target);
...
Goblin.registerQuest(goblinName, 'server.start', function*(
  ...
  const cfg = setWebpackConfig(goblin, null, options.target);
  quest.goblin.setX('config', cfg);
  const config = cfg.toJS();
