...
Goblin.registerQuest(goblinName, 'server.start', function(
  ...
  if (process.env.NODE_ENV !== 'production') {
    options.execArgv = ['--inspect=13229'];
  }

  const resp = busClient.newResponse('webpack', 'token');
  const xProcess = require('xcraft-core-process')({
    logger: 'xlog',
    resp,
  });

  const forked = xProcess.fork(
    childModule,
    ['--only=goblin-webpack'],
    execOptions,
    () => console.log('Webpack dev-server process exited')
  );
  ...
  forked.on('message', message => {
    ...
    switch (msg.type) {
      case 'open':
        quest.cmd('laboratory.open', msg.result);
        break;
      case 'job-done':
        quest.evt(`${msg.result.jobId}.done`);
        break;
      case 'error':
