...
const logicHandlers = {
  ...
  'server.start': (state, action) => {
    ...
    let plugins = [
      ...
      //new webpack.ContextReplacementPlugin (/goblin/, '.', false, /.*\.jsx?$/),
    ];
...
Goblin.registerQuest (goblinName, 'server.start', function (quest, msg) {
  quest.do ();
  const compiledGoblin = msg.get ('goblin');
  ...
  compiler.plugin ('done', stats => {
    quest.log.info (`compiler ${compiledGoblin}.done`);
    quest.evt (`${compiledGoblin}.done`);
  });
...
Goblin.registerQuest (goblinName, 'server.stop', function (quest, msg) {
  const server = getServer (msg.get ('goblin'));
  if (server) {
...
Goblin.registerQuest (goblinName, '_save.stats', function (quest, msg) {
  quest.do ();
