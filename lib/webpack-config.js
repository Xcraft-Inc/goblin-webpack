...
module.exports = {
  setWebpackConfig: (
    ...
    state = state
      ...
      .set(
        'context',
        contextPath.replace(/^[a-z]:\\/, (match) => match.toUpperCase())
      )
      .set(`entry`, [
        indexPath.replace(/^[a-z]:\\/, (match) => match.toUpperCase()),
      ])
    ...
    const nodeModules = path
      .resolve(releasePath || path.join(__dirname, '../../..'), 'node_modules')
