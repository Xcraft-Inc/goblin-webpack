...
const handleMessage = watt(function*(payload, next) {
  ...
  if (fs.existsSync(path.join(outputPath, `/${config.output.filename}`))) {
    ...
    const bundle = fs.readFileSync(path.join(outputPath, 'main.bundle.js'));
    mFs.writeFileSync(`/${config.output.filename}`, bundle);
    mFs.readFileSync(`/${config.output.filename}`);
    process.send({type: 'job-done', result: {jobId}});
  }
});
